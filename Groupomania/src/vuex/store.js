    ...
    handleGetPosts({ commit }) {
      ...
      return axios
        ...
        .then(({ data }) => {
          ...
          const postsWithUsersPromises = posts.map((post) =>
            axios
              .get(`http://localhost:5000/api/v1/users/${post.authorId}`)
              .then(({ data }) => ({ ...post, author: data.data }))
          );
          Promise.all(postsWithUsersPromises).then((postsWithUsers) => {
            console.log(postsWithUsers)
            commit("SET_POST_SUCCESS", postsWithUsers)
          }
          );
        });
    },
